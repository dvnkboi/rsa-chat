...
const vm = new Vue({
  ...
  methods: {
    ...
    setupSocketListeners() {
      ...
      this.socket.on('ENABLE_ENC', () => {
        this.enableEnc();
      });

      this.socket.on('DISABLE_ENC', (key) => {
        this.disableEnc();
      });
    ...
    toggleEnc() {
      if (this.encryptionEnabled) {
        this.socket.emit('DISABLE_ENC');
        this.disableEnc();
      }
      else {
        this.socket.emit('ENABLE_ENC');
        this.enableEnc();
      }
    },
    enableEnc() {
      ...
      this.getWebWorkerResponse('enable-encryption');
      this.addNotification(`ENCRYPTION ENABLED`);
    },
    disableEnc() {
      if (!this.socket) return;
      this.encryptionEnabled = false;
      this.getWebWorkerResponse('disable-encryption');
      this.addNotification(`ENCRYPTION DISABLED`);
    },
